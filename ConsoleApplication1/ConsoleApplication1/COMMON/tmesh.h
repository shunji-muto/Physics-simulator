...
public:
  ...
  TTriangle(){
    verts_[0] = verts_[1] = verts_[2] = EVec3f(0, 0, 0);
  }
  ...
  void Set(const TTriangle &src){
    verts_[0] = src.verts_[0]; 
    verts_[1] = src.verts_[1]; 
    verts_[2] = src.verts_[2];   
  }
...
public:
  int        num_triangles_;
  TTriangle *triangles_   ;
  EVec3f    *normals_  ;

  void Clear(){
    if( triangles_ != 0 ) delete[] triangles_; 
  ...
  void Set(const TTriangleSoup &src)
  {
    ...
    num_triangles_ = src.num_triangles_;
    if( num_triangles_ == 0 ) return;

    triangles_ = new TTriangle[num_triangles_];
    normals_   = new EVec3f   [num_triangles_];

    for( int i = 0 ; i < num_triangles_ ; ++i)
    {
      triangles_[i] = src.triangles_[i];
      normals_[i] = src.normals_[i];
    }
  }
  ...
  void Allocate(int num_triangles, EVec3f *triangle_vertex_array = 0)
  {
    ...
    for(int i = 0; i <  num_triangles_; ++i )
    {
      triangles_[i].verts_[0] = triangle_vertex_array[3*i+0];
      triangles_[i].verts_[1] = triangle_vertex_array[3*i+1];
      triangles_[i].verts_[2] = triangle_vertex_array[3*i+2];
    }
  ...
  {
#pragma omp parallel for
    for (int i = 0; i < num_triangles_ ; ++i) normals_[i].setZero();

    for (int i = 0; i < num_triangles_ ; ++i)
    {
      TTriangle &t = triangles_[i];

      normals_[i] = (t.verts_[1] - t.verts_[0]).cross(t.verts_[2] - t.verts_[0]);
      float l = normals_[i].norm();
      if( l != 0 ) normals_[i] /= l;
    }
  }
  ...
  bool ExportAsStl(const char *fname)
  {
    ...
    for (int i = 0; i < num_triangles_; ++i)
    {
      const TTriangle &t = triangles_[i];
      fprintf(fp, "facet normal %f %f %f\n", normals_[i][0], normals_[i][1], normals_[i][2]);
      fprintf(fp, "  outer loop\n");
      fprintf(fp, "    vertex %f %f %f\n", t.verts_[0][0], t.verts_[0][1], t.verts_[0][2]);
      fprintf(fp, "    vertex %f %f %f\n", t.verts_[1][0], t.verts_[1][1], t.verts_[1][2]);
      fprintf(fp, "    vertex %f %f %f\n", t.verts_[2][0], t.verts_[2][1], t.verts_[2][2]);
      fprintf(fp, "  endloop\n");
      fprintf(fp, "endfacet\n");
    }
    fprintf(fp, "endsolid tmesh\n");
  ...
  void Draw() const
  {
    ...
    {
      const TTriangle& t = triangles_[i];
      glNormal3fv( normals_[i].data() );
      glVertex3fv( t.verts_[0].data() );
      glVertex3fv( t.verts_[1].data() );
      glVertex3fv( t.verts_[2].data() );
    }
    glEnd();
  }
  ...
  void DrawEdges()
  {
    ...
    {
      const TTriangle& t = triangles_[i];
      glVertex3fv( t.verts_[0].data() ); glVertex3fv( t.verts_[1].data() );
      glVertex3fv( t.verts_[1].data() ); glVertex3fv( t.verts_[2].data() );
      glVertex3fv( t.verts_[2].data() ); glVertex3fv( t.verts_[0].data() );
    }
    glEnd();
  }
  ...
  void ImportObj( const char* fname )
  {
    ...
    for( int t = 0; t < mesh.m_pSize; ++t)
    {
      ...
      triangles_[t].verts_[0] = mesh.m_vVerts[idx[0]];
      triangles_[t].verts_[1] = mesh.m_vVerts[idx[1]];
      triangles_[t].verts_[2] = mesh.m_vVerts[idx[2]];
    }
    UpdateNormal();
  }
  ...
  EVec3f GetGravityCenter()
  {
    ...
    {
      gc += triangles_[t].verts_[0];
      gc += triangles_[t].verts_[1];
      gc += triangles_[t].verts_[2];
    }
    return gc / (float)(3 * num_triangles_);
  }
  ...
  void Translate(const EVec3f &trans )
  {
    for( int t = 0; t < num_triangles_; ++t)
    {
      triangles_[t].verts_[0] += trans;
      triangles_[t].verts_[1] += trans;
      triangles_[t].verts_[2] += trans;
    }
  }
  ...
  void MultMat(const EMat4f M)
  {
    ...
    for (int i = 0; i < num_triangles_; ++i) 
    {
      triangles_[i].verts_[0] = R * triangles_[i].verts_[0] + t;
      triangles_[i].verts_[1] = R * triangles_[i].verts_[1] + t;
      triangles_[i].verts_[2] = R * triangles_[i].verts_[2] + t;
    }
  }
...
)
{
  ...
  const double py = pitch[1];
  const double pz = pitch[2];
  const int num_tris = mesh.num_triangles_;
  const TTriangle *tris  = mesh.triangles_;
  const EVec3f    *norms = mesh.normals_;
...
inline void t_drawTransHandle(const EVec3f &gc, double handleSize, const EVec3f &offset)
{
  ...
  glTranslated((double)offset[0], (double)offset[1], (double)offset[2]);
  glDisable(GL_LIGHTING);

  //y
  glColor3d(0, 1, 0);
  t_DrawCylinder(handleSize, CYL_W);
  glTranslated(0, handleSize, 0);

  t_drawCone(CONE_RATE * handleSize, CONE_W);
  glTranslated(0, -handleSize, 0);
...
inline void t_drawCircleXY(double handleSize)
{
  int RES = 50;
  double step = 2 * M_PI / RES;

  glBegin(GL_LINE_STRIP);
  for (int i = 0; i < RES; ++i) glVertex3d(handleSize * cos(step * i), handleSize * sin(step * i), 0);
  glEnd();
}

inline void t_drawRotHandle(const EVec3f &gc, double handleSize, const EVec3f &offset)
{
  EMat4d M = EMat4d::Identity();
